...
class TestSeacrhLocators:
    ...
    LOCATOR_CLICK_CONTACT_US_BTN = (By.XPATH, '//*[@id="contact"]/div[4]/button/div')
    LOCATOR_FIND_ALERT = ()
...
class OperationsHelper(BasePage):
    ...
    def add_contact_name(self, name):
        logging.info(f"Send '{name}' to element {TestSeacrhLocators.LOCATOR_CONTACT_ENTER_NAME[1]}")
        content_field = self.find_element(TestSeacrhLocators.LOCATOR_CONTACT_ENTER_NAME)
        content_field.clear()
    ...
    def add_contact_email(self, email):
        ...
        content_field = self.find_element(TestSeacrhLocators.LOCATOR_CONTACT_EMAIL)
        content_field.clear()
        content_field.send_keys(email)

    def add_contact_content(self, msg):
        logging.info(f"Send '{msg}' to element {TestSeacrhLocators.LOCATOR_CONTACT_EMAIL[1]}")
        content_field = self.find_element(TestSeacrhLocators.LOCATOR_CONTACT_EMAIL)
        content_field.clear()
        content_field.send_keys(msg)

    def click_contact_us_button(self):
        logging.info("Click contact us button")
        self.find_element(TestSeacrhLocators.LOCATOR_CLICK_CONTACT_US_BTN).click()
